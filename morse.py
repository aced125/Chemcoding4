...
morse_constant = int(input('choose your morse coefficient: 1 or 2?'))
...
def MorsePotential(r1,r2):
    return (1-np.exp(-ld(r1,r2) + morse_constant))**2


def sumofMorsePotential(coordlist):
    
    sum = 0
    for i in range(len(coordlist)):
        for j in range(i+1,len(coordlist)):
            sum+=MorsePotential(coordlist[i],coordlist[j])
...
#print(f'\nconverged potential: {round(sumofMorsePotential(coordlist),5)}')
...
def MorsePotentialsum(list1):
    ...
    return sumofMorsePotential(arraycoordlist)
...
for i in range(iterations_to_search_for):
    ...
    xopt, es = cma.fmin2(MorsePotentialsum, list(np.random.randn(particle_number*3)), 0.5)
...
fn = 'MorsePotential'+str(particle_number)+'particles'+str(morse_constant)+'MorseConstant.xyz'
file = open(fn,'w')
file.write(f'{particle_number}\n')
file.write(f'Optimum geometry of {particle_number} particles minimised by Morse potential\n')
